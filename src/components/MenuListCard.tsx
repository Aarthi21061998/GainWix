...
export default function MenuListCard(props: any) {
  ...
  return (
    <Link href={data?.path || "/default-path"} target="_top">
      <div
        className={cn(
          "flex items-start align-center  gap-3  xs:p-1 rounded-xl hover:bg-[#2D2E4F]",
          className
        )}
        {...prop}
      >
        <div className=" flex-column items-center">
          <div className="flex items-center w-[25vw] h-[10vh] ">
          ...
          {data?.content && (
            <div className="md:text-[13px] font-medium text-[#616874] md:pl-14 -mt-5  xs:text-[3.5vw] md:w-[20vw] xs:w-[80vw]">
              {data?.content}
            </div>
          )}
        </div>
